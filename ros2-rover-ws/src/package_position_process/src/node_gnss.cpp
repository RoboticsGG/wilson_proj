...
class GNSSPublisher : public rclcpp::Node {
    public:
        GNSSPublisher() : Node("gnss_publisher") {
            ...
            // Timer to read data every 1 second
    ...
    private:
        ...
        std::string last_valid_date_ = "UNKNOWN";  // Store last valid date
        ...
        void readSerialData() {
            ...
            if (bytes_read > 0) {
                ...
                if (Json::parseFromStream(reader, ss, &root, &errs)) {
                    ...
                    if (!date_time.empty() && date_time.find(" ") != std::string::npos) {
                        date = date_time.substr(0, date_time.find(" "));
                        time = date_time.substr(date_time.find(" ") + 1);
                        last_valid_date_ = date;  // Update last valid date
                    } else {
                        date = last_valid_date_;  // Use last valid date
                    }
