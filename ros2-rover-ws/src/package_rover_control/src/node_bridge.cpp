...
public:
    ...
    {
        // Set Domain ID for Subscriber
        rclcpp::Context::SharedPtr context_sub = std::make_shared<rclcpp::Context>();
        rclcpp::InitOptions init_options_sub;
        init_options_sub.set_domain_id(1);  // Domain ID 1 for Subscriber
        context_sub->init(0, nullptr, init_options_sub);

        sub_node_ = std::make_shared<rclcpp::Node>("sub_node", init_options_sub);
        subscription_ = sub_node_->create_subscription<std_msgs::msg::String>(
            "/topic_a", 10, std::bind(&CombinedNode::topic_callback, this, std::placeholders::_1));
        ...
        init_options_pub.set_domain_id(2);  // Domain ID 2 for Publisher
        context_pub->init(0, nullptr, init_options_pub);

        pub_node_ = std::make_shared<rclcpp::Node>("pub_node", init_options_pub);
        publisher_ = pub_node_->create_publisher<std_msgs::msg::String>("/topic_b", 10);
...
private:
    void topic_callback(const std_msgs::msg::String::SharedPtr msg)
    {
        ...
        RCLCPP_INFO(this->get_logger(), "Published on /topic_b: '%s'", new_msg.data.c_str());
    }
